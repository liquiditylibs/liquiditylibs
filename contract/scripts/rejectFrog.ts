...
async function main() {
    ...
    const { addressOwner, address2, address3, addressUsdl, addressLiquidator } =
        JSON.parse(accounts);
    ...
    const gasReject = await provider.estimateContractGas({
        address: addressLiquidator as `0x${string}`,
        abi: abiLiquidator,
        functionName: "cancel",
        account: addressOwner,
        args: [address2],
    });
    console.log({ frogGas: Number(gasReject) });
    await liquidator.write.cancel([address2], { account: addressOwner });
    const totalGas =
        Number(gasApprove) + Number(gasDeposit) + Number(gasReject);
